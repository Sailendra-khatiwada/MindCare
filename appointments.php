    ...
    <div class="container">
        ...
        <div class="search-filter-bar">
            ...
            <div class="filter-options">
                <a href="#" id="filter-All" class="filter-option active" onclick="filterSpecialization('All')">All Specializations</a>
        ...
        <div class="psychologist-grid" id="psychologistGrid">
                    ...
                    <div class="psychologist-card" data-specialization="<?php echo htmlspecialchars($row['specialization']); ?>" data-location="<?php echo htmlspecialchars(strtolower($row['location'])); ?>">
                        <div class="card-header">
                            <div class="profile-section">
                                <img src="<?php echo !empty($row['profile_picture']) ? htmlspecialchars($row['profile_picture']) : 'images/default-profile.jpg'; ?>" alt="<?php echo htmlspecialchars($row['username']); ?>" class="profile-image">
                        ...
                        <div class="card-body">
                            ...
                            <div class="fees">
                                ...
                                <div class="fee-amount">Rs. <?php echo number_format($row['min_fee']); ?> - <?php echo number_format($row['max_fee']); ?></div>
            ...
            <?php else: ?>
                <div class="empty-state">
                    ...
                    <p class="empty-text">There are currently no psychologists available in our network. Please check back later or contact support for assistance.</p>
                    <a href="dashboard.php" class="action-btn btn-primary" style="width: auto; display: inline-flex;"><i class="fas fa-arrow-left"></i> Back to Dashboard</a>
                </div>
            <?php endif; ?>
        </div>
    </div>
    ...
    <script>
        function filterLocation() {
            ...
            cards.forEach(card => {
                ...
                card.style.display = location.includes(input) ? 'flex' : 'none';
                if (location.includes(input)) card.style.animation = 'fadeIn 0.5s ease-out';
            });
        }
        ...
        function filterSpecialization(specialization) {
            ...
            cards.forEach(card => {
                ...
                card.style.display = (specialization === 'All' || sp.includes(specialization.toLowerCase())) ? 'flex' : 'none';
                if (specialization === 'All' || sp.includes(specialization.toLowerCase())) card.style.animation = 'fadeIn 0.5s ease-out';
            });
