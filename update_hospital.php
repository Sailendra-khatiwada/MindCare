...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("UPDATE hospitals SET name=?, location=?, specialization=?, contact=?, email=?, website=?, description=? WHERE hospital_id=?");
        $stmt->bind_param("sssssssi", $name, $location, $specialization, $contact, $email, $website, $description, $hospital_id);
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <select id="specialization" 
                                    ...
                                    <option value="Psychiatry" <?php echo ($hospital['specialization'] ?? '') == 'Psychiatry' ? 'selected' : ''; ?>>Psychiatry</option>
                                    <option value="Psychology" <?php echo ($hospital['specialization'] ?? '') == 'Psychology' ? 'selected' : ''; ?>>Psychology</option>
                                    <option value="Counseling" <?php echo ($hospital['specialization'] ?? '') == 'Counseling' ? 'selected' : ''; ?>>Counseling</option>
                                    <option value="Rehabilitation" <?php echo ($hospital['specialization'] ?? '') == 'Rehabilitation' ? 'selected' : ''; ?>>Rehabilitation</option>
                                    <option value="Mental Health" <?php echo ($hospital['specialization'] ?? '') == 'Mental Health' ? 'selected' : ''; ?>>Mental Health</option>
                                    <option value="Psychotherapy" <?php echo ($hospital['specialization'] ?? '') == 'Psychotherapy' ? 'selected' : ''; ?>>Psychotherapy</option>
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <input type="tel" 
                                       id="contact" 
                                       name="contact" 
                                       class="form-input <?php echo isset($errors['contact']) ? 'error' : ''; ?>"
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <textarea id="description" 
                                      ...
                                      class="form-textarea"
                                      rows="4"
                                      placeholder="Enter a detailed description of the hospital, services offered, facilities, etc."><?php echo htmlspecialchars($hospital['description'] ?? ''); ?></textarea>
                            <div class="form-hint">
                                <i class="fas fa-info-circle"></i>
                                Optional: Provide additional information about the hospital
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            form.addEventListener('submit', function(e) {
                ...
                if (!isValid) {
                    e.preventDefault();
                    showFormError('Please fix the errors in the form before submitting.');
                }
            });
            ...
            function validateField(field) {
                ...
                if (field.id === 'contact' && field.value) {
                    const contactRegex = /^[\d\s\-\+\(\)]{10,20}$/;
                    if (!contactRegex.test(field.value)) {
                        showFieldError(field, 'Please enter a valid contact number (10-20 digits)');
                        return false;
                    }
                }
            ...
            if (descriptionTextarea) {
                ...
                function updateCounter() {
                    ...
                    counter.className = `char-counter ${length > 500 ? 'exceeded' : ''}`;
                }
