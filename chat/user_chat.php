    ...
    <script>
        ...
        function showNotification(message) {
            ...
            toast.innerHTML = `
                ...
                <div>
                    <strong>New Message from Therapist</strong>
                    <small>${message}</small>
                </div>
            `;
        ...
        function loadChat() {
            fetch("load_messages.php?appointment_id=<?php echo $appointment_id; ?>")
                .then(res => res.json())
                .then(messages => {
                    // Check for new unread psychologist messages and show notification
                    let newUnreadMessages = 0;
                    messages.forEach((m) => {
                        if (m.sender_type === "psychologist" && m.seen == 0 && new Date(m.created_at).getTime() > lastMessageTimestamp) {
                            newUnreadMessages++;
                        }
                    });
                    
                    if (newUnreadMessages > 0) {
                        showNotification(`Your therapist sent ${newUnreadMessages} message${newUnreadMessages > 1 ? 's' : ''}`);
